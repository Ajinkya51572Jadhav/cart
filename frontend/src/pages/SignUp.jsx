...
const SignUp= () => {
  ...
  const [handleData,setHandleData]=useState({name:"",email:"",password:"",confirmpassword:"",profile:"ajinkyalogo.png"});
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
       ...
       if(handleData?.password === handleData?.confirmpassword){
        ...
        try{
          const  {data} = await axios.post(Api.signup.url,{name,email,password,profile});
  ...
  return (
    ...
    <div class="w-20 h-20 mx-auto">
        <img src={handleData?.profile?handleData?.profile:LoginIcon} alt="profile" title="profile" class="rounded-full cursor-pointer"/>
        <form className="-mt-12">
            <input type="file" onChange={handleImage} className="opacity-0 cursor-pointer"/>
        </form>
    </div>
    <div class="text-xs py-1 text-center"> 
        Upload Profile  
    </div>
</div>
